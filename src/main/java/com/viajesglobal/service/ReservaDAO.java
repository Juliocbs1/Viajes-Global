...
public class ReservaDAO implements ReservaMethod {
    ...
    public String saveReserva(ReservaDTO reservaDTO) {
        Reserva reserva;
        if(reservaDTO.getIdPaquete() == null) {
            reserva = new Reserva(reservaDTO.getIdUsuario(), null, reservaDTO.getEstado(), reservaDTO.getTotalPago(), reservaDTO.getIdVuelo(), reservaDTO.getCantidadAiento());
        }else{
            reserva = new Reserva(reservaDTO.getIdUsuario(), reservaDTO.getIdPaquete(), reservaDTO.getEstado(), reservaDTO.getTotalPago(), reservaDTO.getIdVuelo(), reservaDTO.getCantidadAiento());
        }
        try {
            reservaRepository.save(reserva);
            return "Reserva Guardada!";
        } catch (Exception e) {
            return "Error en la reserva " +e.getMessage();
        }
    ...
    public List<ReservaDTO> getReservas() {
        ...
        for(Reserva reserva : reservas) {
            reservasDTO.add(new ReservaDTO(reserva.getIdUsuario(),reserva.getIdPaquete(),reserva.getEstado(),reserva.getTotalPago(),reserva.getIdVuelo(),reserva.getCantidadAiento()));
        }
        return reservasDTO;
    }
}
